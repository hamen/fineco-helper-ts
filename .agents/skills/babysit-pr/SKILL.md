...
allowed-tools: Bash(python3 */skills/babysit-pr/scripts/*), Bash(gh pr *), Bash(gh run *), Bash(gh api *), Bash(git fetch *), Bash(git rebase *), Bash(git merge *), Bash(git checkout *), Bash(git switch *), Bash(git push *), Bash(git commit *), Bash(git diff *), Bash(git log *), Bash(git status), Bash(git branch *), Bash(git worktree *), Bash(npm test), Bash(npm run *), Bash(npx tsc *), Read, Edit
---
...
- PR number — e.g. `123`
- PR URL — e.g. `https://github.com/rock3r/fineco-helper/pull/123`

## Core workflow

0. **Before running any script**, output a single line so the user knows which PR this conversation is tracking — e.g. `Babysitting PR [#123](https://github.com/rock3r/fineco-helper/pull/123)`. Resolve the PR number/URL from the user's input or the current branch first if needed.
...
python3 .agents/skills/babysit-pr/scripts/gh_pr_watch.py --pr https://github.com/rock3r/fineco-helper/pull/42 --snapshot
```
...
| `stop_bugbot_not_green` | Cursor Bugbot is required via `--require-bugbot` and is not SUCCESS (`neutral`, `skipped`, `failure`, or missing) — do not merge |
...
Each push can trigger new CI and bot review runs. **Never push until all of the following are true:**

1. `npm test` and `npm run build` pass locally (no CI failures to fix after the push).
2. No enabled review bot is still `IN_PROGRESS` — wait for active bots to finish so their comments, if any, can be collected and fixed in the same push.
3. You have incorporated all currently visible actionable bot comments into the pending local fix batch.
...
3. Run `npm test` and `npm run build` to confirm green.
...
## Bugbot + Codex merge gates

Never merge while Codex is actively reviewing. Cursor Bugbot is optional by default; pass
`--require-bugbot` when the repository uses Bugbot as a mandatory merge gate.

### Bugbot (CI check)

For Bugbot, only `SUCCESS` is a green gate when `--require-bugbot` is enabled.

- If Bugbot is still `IN_PROGRESS` → keep polling, do not push.
- If Bugbot conclusion is `NEUTRAL` → it found potential issues. Read the inline PR review comments left by `cursor[bot]`, fix every reported issue locally, run `npm test` and `npm run build`, then push once (see push discipline above).
...
- Do **not** merge on NEUTRAL or SKIPPING when Bugbot is required, even if all other checks pass.
...
- **Branch-related failure**: edit the code, collect all other pending issues (bot and human reviews), fix everything, run `npm test` and `npm run build`, then push once.
...
When working from a git worktree, verify you are editing the PR branch worktree,
not the main checkout, before committing or pushing. Always rerun the local
validation commands after rebasing or resolving conflicts.
...
### Harness only returns output after tool exit
